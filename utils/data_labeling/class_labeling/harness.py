# utils/data_labeling/class_labeling/harness.py
...
def main():
    ...
    ap.add_argument("--project", required=True, help="e.g., workspace/harness-xmpgb")
    ...
    for i, img_path in enumerate(images, 1):
        ...
        total_written += n
        print(f"[harness] {i:04d}/{len(images)} {img_path.name}: sp={len(sp_dets)} -> wrote={n}")

    print(f"[harness] done. wrote {total_written} boxes total to {labels_out}")
